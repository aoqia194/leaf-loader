...
import dev.aoqia.leaf.loader.impl.util.*;
import dev.aoqia.leaf.loader.impl.util.log.Log;
import dev.aoqia.leaf.loader.impl.util.log.LogCategory;

import org.spongepowered.asm.mixin.transformer.IMixinTransformer;

final class KnotClassDelegate<T extends ClassLoader & ClassLoaderAccess>
    implements KnotClassLoaderInterface {
    private static final boolean LOG_CLASS_LOAD = SystemProperties.isSet(
        SystemProperties.DEBUG_LOG_CLASS_LOAD);
    private static final boolean LOG_CLASS_LOAD_ERRORS =
        LOG_CLASS_LOAD || SystemProperties.isSet(SystemProperties.DEBUG_LOG_CLASS_LOAD_ERRORS);
    private static final boolean LOG_TRANSFORM_ERRORS = SystemProperties.isSet(
        SystemProperties.DEBUG_LOG_TRANSFORM_ERRORS);
    private static final boolean DISABLE_ISOLATION = SystemProperties.isSet(
    ...
    // provider has to set it to opt in
    private final Map<Path, String[]> allowedPrefixes = new ConcurrentHashMap<>();
    private final Set<String> parentSourcedClasses = Collections.newSetFromMap(
        new ConcurrentHashMap<>());
    private IMixinTransformer mixinTransformer;
    private boolean transformInitialized = false;
    private volatile Set<Path> codeSources = Collections.emptySet();
    private volatile Set<Path> validParentCodeSources = null; // null = disabled isolation, game

    KnotClassDelegate(boolean isDevelopment, EnvType envType, T classLoader,
        ClassLoader parentClassLoader, GameProvider provider) {
    ...
    private static boolean canTransformClass(String name) {
        name = name.replace('/', '.');
        // Blocking Fabric Loader classes is no longer necessary here as they don't exist on the
        // modding class loader
        return /* !"dev.aoqia.api.EnvType".equals(name) && !name.startsWith("dev.aoqia.leaf
        .loader.") && */ !name.startsWith(
    ...
    private static void copyZipEntryIfDistinct(Path zipFile, String fileName, Path output) throws
        IOException {
        try (ZipFile zf = new ZipFile(zipFile.toFile())) {
            ZipEntry entry = zf.getEntry(fileName);
            if (entry == null) {
                throw new FileNotFoundException(
                    String.format("zip file %s doesn't contain %s", zipFile, fileName));
            }
            ...
            if (Files.exists(output)) { // extracted file exists, check size and crc
                ...
                if (Files.size(output) == expectedSize) {
                    ...
                    if (crc.getValue() ==
                        expectedCrc) { // existing file has the same size and crc as the zip entry
                        return;
                    }
                }
            }
    ...
    public void initializeTransformers() {
        ...
        if (mixinTransformer == null) {
            try { // reflective instantiation for older mixin versions
                ...
                Constructor<IMixinTransformer> ctor =
                    (Constructor<IMixinTransformer>) Class.forName(
                            "org.spongepowered.asm.mixin.transformer.MixinTransformer")
                        .getConstructor();
                ctor.setAccessible(true);
                mixinTransformer = ctor.newInstance();
            } catch (ReflectiveOperationException e) {
                Log.debug(LogCategory.KNOT,
                    "Can't create Mixin transformer through reflection (only applicable for 0.8-0" +
                    ".8.2): %s",
                    e);

                // both lookups failed (not received through IMixinService.offer and not found
                // through reflection)
    ...
    Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
        synchronized (classLoader.getClassLoadingLockFwd(name)) {
            ...
            if (c == null) {
                if (name.startsWith(
                    "java.")) { // fast path for java.** (can only be loaded by the platform CL
                    // anyway)
                    ...
                    if (c == null) { // not available locally, try system class loader
                        ...
                        if (url == null) { // no .class file
                            try {
                                ...
                                if (LOG_CLASS_LOAD) {
                                    Log.info(LogCategory.KNOT,
                                        "loaded resources-less class %s from platform class " +
                                        "loader");
                                }
                            ...
                            // The class would technically be available, but the game provider
                            // restricted it from being
                            // loaded by setting validParentUrls and not including "url". Typical
                            // causes are:
                            // - accessing classes too early (game libs shouldn't be used until
                            // Loader is ready)
                            // - using jars that are only transient (deobfuscation input or
                            // pass-through installers)
                            String msg = String.format(
                                "can't load class %s at %s as it hasn't been exposed to the game " +
                                "(yet? The system property " +
                                SystemProperties.PATH_GROUPS + " may not be set correctly in-dev)",
                            ...
                            if (LOG_CLASS_LOAD) {
                                Log.info(LogCategory.KNOT,
                                    "loading class %s using the parent class loader", name);
                            }
     ...
     *
     * <p>This handles explicit parent url whitelisting by {@link #validParentCodeSources} or
     * shadowing by {@link #codeSources}
     */
    private boolean isValidParentUrl(URL url, String fileName) {
        ...
        if (validParentCodeSources !=
            null) { // explicit whitelist (in addition to platform cl classes)
            return validParentCodeSources.contains(codeSource) ||
                   PLATFORM_CLASS_LOADER.getResource(fileName) != null;
    ...
    Class<?> tryLoadClass(String name, boolean allowFromParent) throws ClassNotFoundException {
        ...
        if (!allowedPrefixes.isEmpty() &&
            !DISABLE_ISOLATION) { // check prefix restrictions (allows exposing libraries
            // partially during startup)
            String fileName = LoaderUtil.getClassFileName(name);
        ...
        if (!allowFromParent &&
            !parentSourcedClasses.isEmpty()) { // propagate loadIntoTarget behavior to its nested
            // classes
            int pos = name.length();
        ...
        // The class we're currently loading could have been loaded already during Mixin
        // initialization triggered by `getPostMixinClassByteArray`.
        // If this is the case, we want to return the instance that was already defined to avoid
        // attempting a duplicate definition.
        ...
        if (pkgDelimiterPos > 0) {
            ...
            if (classLoader.getPackageFwd(pkgString) == null) {
                try {
                    classLoader.definePackageFwd(pkgString, null, null, null, null, null, null,
                        null);
        ...
        return metadataCache.computeIfAbsent(codeSource, (Path path) -> {
            ...
            try {
                if (Files.isDirectory(path)) {
                    ...
                    if (manifest == null) {
                        try (FileSystemUtil.FileSystemDelegate jarFs =
                                 FileSystemUtil.getJarFileSystem(
                                     path, false)) {
                            manifest = ManifestUtil.readManifestFromBasePath(
                                jarFs.get().getRootDirectories().iterator().next());
                        }
                    }
    ...
    private byte[] getRawClassByteArray(String name, boolean allowFromParent) throws IOException {
        ...
        if (url == null) {
            ...
            if (!isValidParentUrl(url, name)) {
                if (LOG_CLASS_LOAD) {
                    Log.info(LogCategory.KNOT,
                        "refusing to load class %s at %s from parent class loader", name,
    ...
    /**
     * Implementation of {@link ClassLoader#findLibrary} that falls back to grabbing natives from
     * the class path.
     *
    ...
    synchronized String findLibrary(String libname) {
        String ret = PROCESSED_NATIVES.get(
            libname); // cache for repeat queries, avoids expensive validation
        if (ret != null) {
            return ret;
        }
        ...
        if (Files.isDirectory(codeSource)) { // cp entry is a folder, use library file directly
            ...
            try {
                cacheDir = LeafLoaderImpl.INSTANCE.getGameDir()
                    .resolve(LeafLoaderImpl.CACHE_DIR_NAME)
                    .resolve("natives");
                ...
                Log.warn(LogCategory.KNOT, "Error creating natives cache directory %s", cacheDir,
                    e);
                return null;
            }

            libFile = cacheDir.resolve(fileName);
            Log.debug(LogCategory.KNOT, "Extracting native %s from class path %s to %s", libname,
                url, libFile);
    ...
    interface ClassLoaderAccess {
        ...
        Package definePackageFwd(String name, String specTitle, String specVersion,
