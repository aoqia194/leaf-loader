...
public final class LibClassifier<L extends Enum<L> & LibraryType> {
    private static final boolean DEBUG = SystemProperties.isSet(
    ...
    public LibClassifier(Class<L> cls, EnvType env, GameProvider gameProvider) throws IOException {
        ...
        processManifestClassPath(LoaderLibrary.SERVER_LAUNCH, env,
            junitRun); // not used by fabric itself, but others add Log4J this way
    }
    ...
    private static <L extends Enum<L>> Set<L> makeSet(L[] libs) {
        ...
        ret.addAll(Arrays.asList(libs).subList(1, libs.length));

        return ret;
    }

    private void processManifestClassPath(LoaderLibrary lib, EnvType env, boolean junitRun) throws
        IOException {
        if (lib.path == null || !lib.isApplicable(env, junitRun) ||
            !Files.isRegularFile(lib.path)) {
            return;
        }
    ...
    private void process(Path path, Set<L> excludedLibs) throws IOException {
        ...
        if (Files.isDirectory(path)) {
            for (L lib : libs) {
                if (excludedLibs.contains(lib) || origins.containsKey(lib)) {
                    continue;
                }
    ...
    private void addLibrary(L lib, Path originPath, String localPath) {
        ...
        if (DEBUG) {
            Log.info(LogCategory.LIB_CLASSIFICATION, "%s %s (%s)", lib.name(), originPath,
                localPath);
        }
    }
    ...
    public final boolean is(Path path, L... libs) {
        for (L lib : libs) {
            if (path.equals(origins.get(lib))) {
                return true;
            }
        }
    ...
    public boolean remove(Path path) {
        if (unmatchedOrigins.remove(path)) {
            return true;
        }
