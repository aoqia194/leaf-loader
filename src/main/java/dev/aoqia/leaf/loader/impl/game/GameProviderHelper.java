...
public final class GameProviderHelper {
    ...
    public static Path getEnvGameJar(EnvType env) {
        return getGameJar(env == EnvType.CLIENT ? SystemProperties.GAME_JAR_PATH_CLIENT
    ...
    private static Path getGameJar(String property) {
        ...
        if (!Files.exists(path)) {
            throw new RuntimeException("Game jar " + path + " (" + LoaderUtil.normalizePath(path) +
                                       ") configured through " + property +
    ...
    public static FindResult findFirst(List<Path> paths, Map<Path, ZipFile> zipFiles,
        boolean isClassName, String... names) {
    ...
    public static Map<String, Path> deobfuscate(Map<String, Path> inputFileMap, String gameId,
        ...
        if (!mappingConfig.matches(gameId, gameVersion)) {
            ...
            throw new FormattedException("Incompatible mappings",
                String.format(
                    "Supplied mappings for %s %s are incompatible with %s %s, this is likely " +
                    "caused by launcher misbehavior",
        ...
        for (Map.Entry<String, Path> entry : inputFileMap.entrySet()) {
            ...
            if (Files.exists(tmpFile)) { // previous unfinished remap attempt
                Log.warn(LogCategory.GAME_REMAP,
                    "Incomplete remapped file found! This means that the remapping process failed" +
                    " on the previous launch. If this persists, make sure to let us at Fabric " +
                    "know!");
        ...
        if (!emittedInfo) {
            Log.info(LogCategory.GAME_REMAP,
        ...
        try {
            ...
            deobfuscate0(inputFiles, outputFiles, tmpFiles, mappingConfig.getMappings(),
                sourceNamespace, targetNamespace, launcher);
        } catch (IOException e) {
    ...
    private static Path getDeobfJarDir(Path gameDir, String gameId, String gameVersion) {
        Path ret = gameDir.resolve(LeafLoaderImpl.CACHE_DIR_NAME)
            .resolve(LeafLoaderImpl.REMAPPED_JARS_DIR_NAME);
        ...
        versionDirName.append(LeafLoaderImpl.VERSION);

        return ret.resolve(versionDirName.toString().replaceAll("[^\\w\\-\\. ]+", "_"));
    }

    private static void deobfuscate0(List<Path> inputFiles, List<Path> outputFiles,
        List<Path> tmpFiles, MappingTree mappings, String sourceNamespace, String targetNamespace,
        LeafLauncher launcher) throws IOException {
        TinyRemapper remapper = TinyRemapper.newRemapper(
                new TinyRemapperLoggerAdapter(LogCategory.GAME_REMAP))
            .withMappings(
                TinyUtils.createMappingProvider(mappings, sourceNamespace, targetNamespace))
        ...
        if (!missing.isEmpty()) {
            throw new RuntimeException(
                "Generated deobfuscated JARs contain no classes: " + missing);
        }
    }
