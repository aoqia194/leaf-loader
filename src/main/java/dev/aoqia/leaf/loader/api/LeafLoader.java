...
public interface LeafLoader {
     ...
     * to a {@link LanguageAdapter} to offer an instance of entrypoint. It is either a string, or an
     ...
     *   }
     * </blockquote></pre>
     * where the {@code default} adapter is the {@linkplain LanguageAdapter adapter} offered by
     * Fabric Loader. </p>
     *
     * @param key the key in entrypoint declaration in {@code leaf.mod.json}
     * @param type the type of entrypoints
     ...
     * {@link #getEntrypointContainers(String, Class)} for the given
     * <code>key</code> and <code>type</code>.
     *
     * <p>The action is invoked by applying the given <code>invoker</code> to each entrypoint
     * instance.
     *
     * <p>Exceptions thrown by <code>invoker</code> will be collected and thrown after all
     * entrypoints have been invoked.
     *
     * @param key the key in entrypoint declaration in {@code leaf.mod.json}
     * @param type the type of entrypoints
     ...
     *
     * <p>The share allows mods to exchange data without directly referencing each other. This
     * makes simple interaction easier by eliminating any compile- or run-time dependencies if the
     * shared value type is independent of the mod (only Java/game/Fabric types like collections,
     * primitives, String, Consumer, Function, ...).
     *
     * <p>Active interaction is possible as well since the shared values can be arbitrary Java
     * objects. For example exposing a {@code Runnable} or {@code Function} allows the "API" user to
     * directly invoke some program logic.
     *
     * <p>It is required to prefix the share key with the mod id like {@code mymod:someProperty}.
     * Mods should not modify entries by other mods. The share is thread safe.
     *
    ...
    /**
     * Checks if Fabric Loader is currently running in a "development" environment. Can be used for
     * enabling debug mode or additional checks.
     *
     ...
     *
     * @return whether or not Loader is currently in a "development" environment
     */
    ...
    /**
     * Get the current game instance. Can represent a game client or server object. As such, the
     * exact return is dependent on the current environment type.
     *
     * <p>The game instance may not always be available depending on the game version and
     * {@link EnvType environment}.
     *
     ...
     *
     * <p>The implementation will try to strip or obscure sensitive data like authentication tokens
     * if {@code sanitize} is set to true. Callers are highly encouraged to enable sanitization as
     * compromising the information can easily happen with logging, exceptions, serialization or
     * other causes.
     *
     * <p>There is no guarantee that {@code sanitize} covers everything, so the launch arguments
     * should still not be logged or otherwise exposed routinely even if the parameter is set to
     * {@code true}. In particular it won't necessarily strip all information that can be used to
     * identify the user.
     *
