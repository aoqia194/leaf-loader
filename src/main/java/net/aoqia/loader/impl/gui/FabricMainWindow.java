...
import net.aoqia.loader.impl.gui.LeafStatusTree.LeafBasicButtonType;
import net.aoqia.loader.impl.gui.LeafStatusTree.LeafStatusButton;
import net.aoqia.loader.impl.gui.LeafStatusTree.Leaf;
import net.aoqia.loader.impl.gui.LeafStatusTree.LeafStatusTab;
...
class FabricMainWindow {
	...
	private static void createUi(CountDownLatch onCloseLatch, LeafStatusTree tree) {
		...
		if (tree.tabs.isEmpty()) {
			LeafStatusTab tab = new LeafStatusTab("Opening Errors");
			...
			LeafStatusTab tab = tree.tabs.get(0);
			contentPane.add(createTreePanel(tab.node, tab.filterLevel, icons), BorderLayout.CENTER);
		} else {
			JTabbedPane tabs = new JTabbedPane();
			contentPane.add(tabs, BorderLayout.CENTER);

			for (LeafStatusTab tab : tree.tabs) {
		...
		if (!tree.buttons.isEmpty()) {
			...
			for (LeafStatusButton button : tree.buttons) {
				JButton btn = new JButton(button.text);
				buttons.add(btn);
				btn.addActionListener(event -> {
					if (button.type == LeafBasicButtonType.CLICK_ONCE) btn.setEnabled(false);
	...
	private static JPanel createTreePanel(Leaf rootNode, LeafTreeWarningLevel minimumWarningLevel,
			IconSet iconSet) {
	...
	static final class IconInfo {
		...
		public static IconInfo fromNode(Leaf node) {
	...
	static class CustomTreeNode implements TreeNode {
		...
		CustomTreeNode(TreeNode parent, Leaf node, LeafTreeWarningLevel minimumWarningLevel) {
			this.parent = parent;
			this.node = node;

			for (Leaf c : node.children) {
