...
public final class LeafTransformer {
	public static byte[] transform(boolean isDevelopment, EnvType envType, String name, byte[] bytes) {
		...
		boolean applyAccessWidener = isZomboidClass && LeafLoaderImpl.INSTANCE.getAccessWidener().getTargets().contains(name);
		...
		if (applyAccessWidener) {
			visitor = AccessWidenerClassVisitor.createClassVisitor(LeafLoaderImpl.ASM_VERSION, visitor, LeafLoaderImpl.INSTANCE.getAccessWidener());
			visitorCount++;
		}
