...
public final class GameProviderHelper {
    ...
    public static Path getCommonGameJar() {
        return getGameJar(SystemProperties.GAME_PATH);
    }

    public static Path getEnvGameJar(EnvType env) {
        return getGameJar(
                env == EnvType.CLIENT ? SystemProperties.GAME_PATH_CLIENT : SystemProperties.GAME_PATH_SERVER);
    }
    ...
    private static Path getGameJar(String property) {
        ...
        if (!Files.exists(path)) {
            throw new RuntimeException(
                    "Game jar " + path + " (" + LoaderUtil.normalizePath(path) + ") configured through " + property +
    ...
    public static FindResult findFirst(List<Path> paths, Map<Path, ZipFile> zipFiles, boolean isClassName,
            String... names) {
    ...
    public static Map<String, Path> deobfuscate(Map<String, Path> inputFileMap, String gameId, String gameVersion,
            Path gameDir, FabricLauncher launcher) {
        return deobfuscate(inputFileMap, gameId, gameVersion, gameDir, launcher, "official");
    }

    public static Map<String, Path> deobfuscate(Map<String, Path> inputFileMap, String gameId, String gameVersion,
            Path gameDir, FabricLauncher launcher, String sourceNamespace) {
        ...
        if (!mappingConfig.matches(gameId, gameVersion)) {
            ...
            throw new FormattedException("Incompatible mappings", String.format(
                    ...
                    (mappingsGameVersion != null ? mappingsGameVersion : "(unknown)"), gameId, gameVersion));
        }
        ...
        try {
            ...
            deobfuscate0(inputFiles, outputFiles, tmpFiles, mappingConfig.getMappings(), sourceNamespace,
                    targetNamespace, launcher);
        } catch (IOException e) {
    ...
    private static Path getDeobfJarDir(Path gameDir, String gameId, String gameVersion) {
        Path ret = gameDir.resolve(LeafLoaderImpl.CACHE_DIR_NAME).resolve(LeafLoaderImpl.REMAPPED_JARS_DIR_NAME);
    ...
    private static void deobfuscate0(List<Path> inputFiles, List<Path> outputFiles, List<Path> tmpFiles,
            MappingTree mappings, String sourceNamespace, String targetNamespace, FabricLauncher launcher) throws
            IOException {
